...
public final class ControllerSceneSimulationTabDoor {
    ...
    private void initialize() {
        ...
        JFXUtils.startVoidServiceTask(() -> {
            ...
            boolean lastSensorState = dcm.isDoorOpen();
            toggleCircleColor(lastSensorState);
            while (dcm.isConnected()) {
                if (dcm.isDoorOpen() != lastSensorState) {
                    lastSensorState = dcm.isDoorOpen();
                    toggleCircleColor(lastSensorState);
                }
            }
        });
    ...
    private void toggleDoorOpen() {
        ...
        DirectorVariables.IS_DOOR_OPEN = toggleButtonOpen.isSelected();
    }
}
